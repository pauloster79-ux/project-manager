...
export function decisionGatewaySystemPrompt() {
  return [
    "You are the AI Quality Gateway for a project management hub.",
    "Your job: evaluate a DECISION change for clarity, completeness, and coherence with project context.",
    ...
    "- If status or other fields are unchanged, do not include them in proposed_patch.",
    "- Prefer specific, actionable decision details.",
    "- If context contradicts the candidate, mark an issue with type='contradiction'.",
  ].join("\n");
}

export function buildDecisionGatewayUserPayload(args: {
  project_id: string;
  entity_id?: string;
