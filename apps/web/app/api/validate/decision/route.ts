...
import { packContextForDecision, packContextForQuestion } from "@/src/lib/retrieval";
import { applyDecisionRules } from "@/src/domain/decision/rules";
import { scoreFromIssues, blockedFromIssues } from "@/src/domain/decision/score";
import { decisionGatewaySystemPrompt, buildDecisionGatewayUserPayload } from "@/src/domain/decision/prompt";
import { z } from "zod";
...
export async function POST(req: Request) {
  ...
  if (entity_id) {
    const { rows } = await query(`select * from decisions where id = $1 and project_id = $2`, [entity_id, project_id]);
    if (!rows[0]) return apiError(404, "Decision not found for validate");
    base = rows[0];
  }
  ...
  const rules = applyDecisionRules(base, candidate, todayISO);

  // 5) Pack retrieval context
  let context;
  if (entity_id) {
    context = await packContextForDecision(project_id, entity_id);
  } else {
    // New decision: use the candidate text to pull helpful snippets
    const q = [candidate.title, candidate.detail].filter(Boolean).join(". ");
    context = await packContextForQuestion(project_id, q || "decision creation");
  }
  ...
  try {
    llm = await completeJSON({
      system: decisionGatewaySystemPrompt(),
      user: buildDecisionGatewayUserPayload({
        project_id,
        entity_id,
        base,
        candidate,
